...
namespace PactNet.Mocks.MockHttpService.Models
{
    public class HttpBodyContent
    {
        ...
        public HttpBodyContent(byte[] content, MediaTypeHeaderValue contentType) : this(contentType)
        {
            ...
            if (IsJsonContentType())
            {
                string jsonContent = Encoding.GetString(content);
                Content = jsonContent;
            ...
            {
                string stringContent = Encoding.GetString(content);
                Content = stringContent;
                Body = stringContent;
            }
        }

        private bool IsJsonContentType() => ContentType.MediaType.IndexOf("application/", StringComparison.OrdinalIgnoreCase) == 0 
            && ContentType.MediaType.IndexOf("json", StringComparison.OrdinalIgnoreCase) > 0;

        private bool IsBinaryContentType() => ContentType.MediaType.IndexOf("application/", StringComparison.OrdinalIgnoreCase) == 0 
