...
namespace PactNet.Mocks.MockHttpService.Comparers
{
    internal class HttpHeaderComparer : IHttpHeaderComparer
    {
        public ComparisonResult Compare(IDictionary<string, string> expected, IDictionary<string, string> actual)
        {
            ...
            foreach (var header in expected)
            {
                ...
                if (actual.TryGetValue(header.Key, out actualValue))
                {
                    ...
                    var actualValueSplit = actualValue.Split(',', ';');
                    ...
                    {
                        var expectedValueSplit = expectedValue.Split(',', ';');
                        string expectedValueSplitJoined = string.Join(",", expectedValueSplit.Select(x => x.Trim()));
                        string actualValueSplitJoined = string.Join(",", actualValueSplit.Select(x => x.Trim()));
        ...
        private IDictionary<string, string> MakeDictionaryCaseInsensitive(IDictionary<string, string> from) => new Dictionary<string, string>(from, StringComparer.OrdinalIgnoreCase);
    }
}