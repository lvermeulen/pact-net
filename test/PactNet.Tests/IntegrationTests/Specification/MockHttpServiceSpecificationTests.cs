...
namespace PactNet.Tests.IntegrationTests.Specification
{
    public class MockHttpServiceSpecificationTests
    {
            ...
            where T : class, IVerifiable
        {
            ...
            {
                throw new InvalidOperationException($"Specification tests not found in path '{pathToTestCases}'. Please ensure pact-specification git submodule has been pulled (git submodule update --init).");
            }

            foreach (string testCaseSubDirectory in Directory.EnumerateDirectories(pathToTestCases))
            {
                var testCaseFileNames = Directory.GetFiles(testCaseSubDirectory);
                foreach (string testCaseFileName in testCaseFileNames)
                {
                    string testCaseJson = File.ReadAllText(testCaseFileName);
